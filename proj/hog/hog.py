...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    return roll_dice(num_rolls, dice) if num_rolls > 0 else piggy_points(opponent_score)
    # END PROBLEM 3
...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    leftmost = lambda x, n=2: str(x).zfill(n)[0:n]
    for pp, oo in zip(list(leftmost(player_score)), list(leftmost(opponent_score))):
        if pp >= oo:
            return False
    return True
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        strategy, player_score, opponent_score = (
        ...
        points = take_turn(num_rolls, opponent_score, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        # n.b. `print` should be the only side effect
        _running_high = running_high
        if score - last_score > running_high:
            _running_high = score - last_score
            print(f"Player {who} has reached a new maximum point gain. {_running_high} point(s)!")
        _last_score = score
        return announce_highest(who, _last_score, _running_high)
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    def repeat(*args):
        results = [original_function(*args) for _ in range(trials_count)]
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    boss_num_rolls, boss_averaged_dice = 0, 0
    for i in range(1, 11):
        val = averaged_dice(i, dice)
        if val > boss_averaged_dice:
            boss_num_rolls = i
            boss_averaged_dice = val
    return boss_num_rolls
    # END PROBLEM 9
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if piggy_points(opponent_score) >= cutoff:
        return 0
    elif more_boar(score + piggy_points(opponent_score), opponent_score):
        return 0
...
def final_strategy(score, opponent_score):
    ...
    return more_boar_strategy(score, opponent_score, cutoff=6, num_rolls=2)
